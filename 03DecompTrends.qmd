...
#| fig-height: 3
tibble(year = as.numeric(time(co2)), co2 = as.numeric(co2)) |>
  ggplot(aes(year, co2)) +
  geom_line(color = sfPal[1]) +
  geom_abline(intercept = coef(co2Lm)[1], slope = coef(co2Lm)[2], color = sfPal[5]) +
...
tibble(year = as.numeric(time(co2Decomp$trend)), trend = as.numeric(co2Decomp$trend)) |>
  ...
  geom_line(color = sfPal[1], na.rm = TRUE) +
  geom_abline(intercept = coef(co2Lm)[1], slope = coef(co2Lm)[2], color = sfPal[5]) +
  labs(
...
azElec |>
  ...
  geom_line() +
  labs(
    x = NULL, y = "Residential sales (GWh)",
