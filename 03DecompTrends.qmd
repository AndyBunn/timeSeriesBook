...
We keep the `tsibble` [@Wang2020tsibble] framework for storing and plotting data, with `tidyverse` [@R-tidyverse] for wrangling and `PNWColors` [@R-PNWColors] for the figure colors, but the decomposition machinery itself lives in base R. The `decompose` function and the `ts` class it works on are both built into R, so there is nothing extra to load for the analysis.
