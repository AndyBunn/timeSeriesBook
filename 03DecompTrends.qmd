...
We keep the tsibble framework for storing and plotting data, but the decomposition machinery itself lives in base R. The `decompose` function and the `ts` class it works on are both built into R, so there is nothing extra to load for the analysis.
...
noise_true  <- rnorm(n, sd = 2)             # the jostle
