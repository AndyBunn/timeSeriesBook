...
trendTrue  <- 10 + 0.05 * tt               # a slow linear climb
seasonTrue <- 4 * sin(2 * pi * tt / 12)    # an annual cycle
noiseTrue  <- rnorm(n, sd = 2)             # the jostle

y <- trendTrue + seasonTrue + noiseTrue
...
mLoop <- rep(NA, n)                         # an empty vector to fill
for (i in 7:(n - 6)) {
  mLoop[i] <- (0.5 * y[i - 6] + y[i - 5] + y[i - 4] + y[i - 3] + y[i - 2] + y[i - 1] +
    y[i] + y[i + 1] + y[i + 2] + y[i + 3] + y[i + 4] + y[i + 5] +
    0.5 * y[i + 6]) / 12
}
mLoop <- ts(mLoop, start = start(y), frequency = 12)
```
...
max(abs(mLoop - m_hat), na.rm = TRUE)
```
...
monthlyMeans <- tapply(sDetrended, cycle(y), FUN = mean, na.rm = TRUE)
monthlyMeans <- monthlyMeans - mean(monthlyMeans)   # center on zero
monthlyMeans
```
...
s_hat <- rep(monthlyMeans, length.out = n)
...
yDecomp <- decompose(y, type = "additive")
```
...
#| fig-height: 5
ggplot(tidyDecomp(yDecomp), aes(time, value)) +
  geom_line(na.rm = TRUE) +
  facet_wrap(~component, ncol = 1, scales = "free_y") +
...
max(abs(yDecomp$trend - m_hat), na.rm = TRUE)
max(abs(yDecomp$seasonal - s_hat), na.rm = TRUE)
```
...
The slope says carbon dioxide rose about `r round(coef(co2Lm)[2], 2)` ppm per year on average over this record. Put that line on the data.

```{r}
#| label: decomp-co2-lm-plot
#| fig-height: 3
plot(co2, ylab = expression(CO[2] ~ (ppm)))
abline(co2Lm, col = sfPal[5])
```
...
#| fig-height: 3
plot(co2Decomp$trend, ylab = expression(CO[2] ~ (ppm)),
...
junkDecomp <- decompose(junk)
ggplot(tidyDecomp(junkDecomp), aes(time, value)) +
  geom_line(na.rm = TRUE) +
  facet_wrap(~component, ncol = 1, scales = "free_y") +
...
elecTs <- ts(azElec$SALES, start = c(2001, 1), frequency = 12)
```
...
#| fig-height: 5
elecDecomp <- decompose(elecTs, type = "multiplicative")
ggplot(tidyDecomp(elecDecomp), aes(time, value)) +
  geom_line(na.rm = TRUE) +
  facet_wrap(~component, ncol = 1, scales = "free_y") +
  labs(x = NULL, y = NULL,
...
1. Take `elecTs` from the body and decompose it the other way too, additively. For each fit, reconstruct the fitted series (`trend + seasonal` for the additive model, `trend * seasonal` for the multiplicative one) and compute the MSE of the residual. Which model fits better, and by how much?
