...
The red line is what ordinary least squares draws through it. Is that trend significantly different from zero?
...
That was the trend question. Now the second kind, the relationship question, and the same trap is waiting. This time we build two series that have nothing to do with each other. Call them `a` and `b`.  Each is its own AR(1) series, drawn separately, so by construction neither has any pull on the other. Then we regress one on the other and ask whether the slope is significant. The answer we should get is no.
...
#| fig-height: 3
ggAcf(residuals(ols_spurious)) + labs(title = "ACF of OLS residuals") +
  theme_minimal()
```
...
spurious_rate <- function(phi, n = 120, sims = 1000) {
  ...
  mean(replicate(sims, {
    a <- ar1_series(phi, n)   # one series
    b <- ar1_series(phi, n)   # a second, unrelated one
...
A few smaller shifts. The coefficient columns are spelled `Value`, `Std.Error`, `t-value`, and `p-value`. The residual five-number summary is labeled `Standardized residuals`, since these are normalized by the fitted covariance. And `gls` prints a `Correlation:` block that `lm` hides; watch out, because that block is the correlation among the coefficient estimates, not the autocorrelation of the residuals. Different thing. The block that does report the error structure is `Correlation Structure: AR(1)`, which gives the estimated `Phi`, the lag-one autocorrelation `gls` fit to the errors. Here it landed near 0.9, close to the 0.85 we built in.
:::

GLS does the same favor for the ghost relationship. We refit the two independent series, now telling `gls` the errors follow an AR(1) process.

```{r}
#| label: reg-spurious-gls
gls_spurious <- gls(b ~ a, correlation = corAR1())
summary(gls_spurious)$tTable
```

The slope that looked so convincing, significant at two in ten million, now carries a $p$-value around 0.15. The ghost is gone. Once GLS stops treating the autocorrelated wobbles of `a` and `b` as independent evidence, two series wandering on their own no longer look related. Both ghosts, the false trend and the false relationship, fall to the same fix.
...
ggplot(air, aes(Day, ozone)) +
  geom_line(color = sfPal[1]) +
  labs(x = "Day of summer", y = "Ozone (ppb)",
       title = "Daily ground-level ozone, New York, summer 1973") +
  theme_minimal()
```

Now let's make and a model.
