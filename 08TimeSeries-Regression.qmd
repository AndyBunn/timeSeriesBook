...
Zooming out a bit. We fit a linear model of the form $y=\beta_0 + \beta_1x$ using GLS and got out estimates for the intercept ($\hat\beta_0$) and slope ($\hat\beta_1$). First, we assumed that the error term was uncorrelated. Looking at summary, we can see the the estimate of the slope ($\hat\beta_1$) is `r round(summary(gls1)$tTable[2,1],3)` and significantly different than zero. We also have a variety of goodness of fit statistics and some other goodies. I'll walk through those in the video. The important thing to wrap your head around here is that we fit GLS with uncorrelated errors meaning it's essentially identical to OLS. Compare the above to the more familiar `summary(lm(y~x))`.
...
2. The original function we used to make $y$ was $y=\beta_0 + \beta_1x + \epsilon$ where $\beta_0 = 0$ and $\beta_1$ = `r B1`. Both the GLS models got the intercept right (`r round(coef(gls2)[1],2)` for `gls2` and `r round(coef(gls1)[1],2)` for `gls1`). But the GLS model with the correlated residuals does a better job of getting the true value of the slope (`r round(coef(gls2)[2],2)` vs `r round(coef(gls1)[2],2)`).

3. The error estimate on $\beta_1$ was lower on the model with correlated residuals than the naive uncorrelated model (`r round(summary(gls2)$tTable[2,2],4)` vs `r round(summary(gls1)$tTable[2,2],4)`). This is because GLS uses the correct structure of error variance and correlation, which improves **efficiency** of parameter estimation.
...
params <- expand_grid(n=rep(c(50,100,150,200),15),
                      phi = rep(c(0.3,0.5,0.7,0.9),15))
params$gls1B1Hat <- 0
params$gls2B1Hat <- 0
B0 <- 0 # intercept
B1 <- 0.5 # slope
for(i in 1:nrow(params)){
  x <- ts(rnorm(params$n[i]))
  epsilon <- arima.sim(model = list(ar=params$phi[i]),n = params$n[i])
  epsilon <- epsilon - mean(epsilon) # demean
  y <- B0 + B1*x + epsilon
  gls1 <- gls(y~x)
  cs1 <- corARMA(p=1)
  gls2 <- update(gls1,correlation=cs1)
  params$gls1B1Hat[i] <- coef(gls1)[2]
  params$gls2B1Hat[i] <- coef(gls2)[2]
}

params <- params %>% 
  mutate(phiFac = as.factor(phi),
         nFac = as.factor(n))

params$phiFac <- recode_factor(params$phiFac,
                               "0.3" = "AR(1)<0.4",
                               "0.5" = "0.4<AR(1)<0.6",
                               "0.7" = "0.6<AR(1)<0.8",
                               "0.9" = "AR(1)>0.8")
...
params2 <-params %>%
  select(gls1B1Hat,gls2B1Hat,phiFac,nFac) %>% 
  rename("Naive OLS" = gls1B1Hat,
         "GLS with AR(1) Corr Struct" = gls2B1Hat) %>%
  pivot_longer(cols = 1:2,names_to = "model",values_to = "B1")

params2 <- params2 %>% filter(between(B1,0,1))
pBetaHat <- ggplot(params2,aes(x=nFac,y=B1)) +
  geom_hline(yintercept = B1) +
  geom_boxplot() + 
  facet_grid(cols=vars(phiFac),rows=vars(model)) +
  labs(x="Sample Size",y=expression(hat(beta[1])),
       title = "Comparison of models under temporal autocorrelation",
       subtitle=expression(y[t]==beta[0]+beta[1]*x[t]+epsilon[t]~where~beta[0]==0~beta[1]==0.5~and~epsilon~follows~an~AR1~process),
       caption = "Results from 3600 MC simulations")
pBetaHat
png(filename = "ols and gls sim with ar1.png",width = 600,height = 400)
pBetaHat
dev.off()

library(ggridges)
pBetaHat2 <- ggplot(params2,aes(y=nFac,x=B1)) +
  geom_density_ridges() +
  geom_vline(xintercept = B1) +  
  facet_grid(cols=vars(phiFac),rows=vars(model)) +
  labs(y="Sample Size",x=expression(hat(beta[1])),
       title = "Comparison of models under temporal autocorrelation",
       subtitle=expression(y[t]==beta[0]+beta[1]*x[t]+epsilon[t]~where~beta[0]==0~beta[1]==0.5~and~epsilon~follows~an~AR1~process),
       caption = "Results from 3600 MC simulations") +
...
pBetaHat2
png(filename = "ols and gls sim with ar1 dens.png",width = 600,height = 400)
pBetaHat2
dev.off()
...
```{r,echo=FALSE,eval=FALSE}
gls1 <- gls(LeesWYflow~OctAprP,data=dat)
