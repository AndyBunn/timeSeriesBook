...
We will use the `tidyverse` [@R-tidyverse] for wrangling and plotting, `tsibble` [@Wang2020tsibble] to store time series as tidy tables, `lubridate` for working with dates, `slider` [@R-slider] for moving averages, and `PNWColors` [@R-PNWColors] for the book's figure colors. Install them if you haven't (e.g. `install.packages(c("tidyverse", "tsibble", "slider", "PNWColors"))`; `lubridate` is part of the tidyverse).
