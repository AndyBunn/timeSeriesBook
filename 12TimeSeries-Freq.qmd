...
ordinates400  <- replicate(500, noiseOrdinate(400))
ordinates4000 <- replicate(500, noiseOrdinate(4000))

tibble(n = factor(rep(c("400", "4000"), each = 500), levels = c("400", "4000")),
       value = c(ordinates400, ordinates4000)) |>
  ggplot(aes(value, fill = n)) +
  geom_histogram(position = "identity", alpha = 0.6, bins = 30) +
  scale_fill_manual(values = c(`400` = sfPal[1], `4000` = sfPal[5])) +
  labs(x = "Periodogram value at frequency 0.1", y = "Count", fill = "n",
       title = "Ten times the data, the same amount of noise") +
  theme_minimal()
```

The two histograms sit right on top of each other. Ten times the data bought nothing: the coefficient of variation (standard deviation divided by mean) is `r round(sd(ordinates400) / mean(ordinates400), 2)` at $n=400$ and `r round(sd(ordinates4000) / mean(ordinates4000), 2)` at $n=4000$. That's not a fluke of this particular simulation; it's a property of the periodogram itself. Each ordinate is built from just two numbers (a sine and a cosine coefficient at that frequency), and adding more data doesn't give any single ordinate more information, it just adds more ordinates alongside it. A longer series gives you a finer-grained frequency axis, not a more precise reading at any one frequency on it.
...
Simulated series are a good place to learn the mechanics, but let's point the same tool at data nobody built, starting with a periodic driver you don't need a telescope to see: the tide outside almost anyone's window on a coastline. `data/cherry_point_tides.csv` holds hourly water level for all of 2023 at Cherry Point, a NOAA tide gauge just north of Bellingham.
...
- [Wavelets](13Wavelets.html) picks up where this chapter's Wrapping Up leaves off: what to do when a series's frequency content changes over time.
