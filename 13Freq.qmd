...
We'll use base R's own `spectrum()` function here, so nothing about a Fourier transform happens out of view. `tidyverse` [@R-tidyverse] does the wrangling and plotting, `PNWColors` [@R-PNWColors] supplies the figure colors, and `cowplot` [@R-cowplot] stacks a time-domain plot on top of its frequency-domain twin.
