...
tidalConstituents <- tibble(
  ...
  mutate(freq    = 1 / period_hr,
         label_y = max(tideSpecDat$spec) * c(1.15, 0.55, 1.15, 0.55),
         label_h = c(-0.3, 1.3, 1.3, -0.3))
...
ggplot(tideSpecDat, aes(freq, spec)) +
  ...
  geom_text(data = tidalConstituents, aes(x = freq, y = label_y, label = label, color = label, hjust = label_h),
