...
Note that Mann-Kendall, meanwhile, sees the trend clearly, with $\tau$ = `r round(mk_planted$tau, 2)` and a $p$-value of `r format(mk_planted$p, scientific = TRUE, digits = 2)`.

```{r}
ts_planted <- theil_sen(x)
ts_planted
```

And that Theil-Sen has the slope nearer the true 0.3 at `r round(ts_planted, 2)`. 
...
tibble(t, x) |>
  ggplot(aes(t, x)) +
  geom_point(color = sfPal[1]) +
  geom_abline(data = tibble(slope = coef(ols_planted)[2], 
                            intercept = coef(ols_planted)[1], 
                            Line = "OLS"),
              aes(slope = slope, intercept = intercept, color = Line),
              linewidth = 0.7) +
  geom_abline(data = tibble(slope = ts_planted, 
                            intercept = median(x) - ts_planted * median(t), 
                            Line = "Theil-Sen"),
              aes(slope = slope, intercept = intercept, color = Line),
              linewidth = 0.7) +
...
So when do you reach for these instead of OLS? Reach for Mann-Kendall and Theil-Sen when you don't trust the trend to be a straight line, when the errors are skewed or prone to outliers, or when you just want a clean read on direction without committing to a parametric model. Stick with OLS, or GLS from the last chapter, when you need an actual model: a slope with a full uncertainty interval, more than one predictor, or a fitted line you plan to use later for forecasting or detrending. The two are not in competition. A common move is to run both: let the rank test confirm the direction and a slope that shrugs off outliers, and let OLS or GLS supply the rest of the inferential machinery.
...
ols_winter <- lm(winter ~ year, data = annual)

ggplot(annual, aes(year, winter)) +
  geom_line(color = sfPal[1]) +
  geom_abline(data = tibble(slope = coef(ols_winter)[2],
                            intercept = coef(ols_winter)[1],
                            Line = "OLS"),
              aes(slope = slope, intercept = intercept, color = Line),
              linewidth = 0.7) +
  geom_abline(data = tibble(slope = ts_winter,
...
Summer flow is sliding the other way, with a negative slope, but Mann-Kendall returns a $p$-value of `r round(mk_summer$p, 2)`, nowhere near significant. The melt-season decline is the half of the fingerprint we would most expect, and on this single gauge it is buried in the noise. 
