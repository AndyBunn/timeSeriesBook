...
fullYears <- nooksack |> count(year) |> filter(n >= 350) |> pull(year)
...
ggplot(annual, aes(year, winter)) +
  ...
  geom_abline(data = tibble(slope = coef(olsWinter)[2],
    intercept = coef(olsWinter)[1],
    Line = "OLS"),
  aes(slope = slope, intercept = intercept, color = Line, linetype = Line),
  linewidth = 0.7) +
  geom_abline(data = tibble(slope = tsWinter,
