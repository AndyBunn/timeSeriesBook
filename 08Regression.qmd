...
We use `tidyverse` [@R-tidyverse] for wrangling and read example data from CSV. The new tool is `gls` from `nlme` [@R-nlme], which fits a linear model while letting the errors carry a correlation structure. We use `forecast` [@R-forecast] for `ggAcf`/`ggPacf`, `ggridges` [@R-ggridges] for the simulation figure, `gridExtra` [@R-gridExtra] to lay a few plots out together, and `PNWColors` [@R-PNWColors] for the book's palette.
