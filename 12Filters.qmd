...
You'll want `dplR` [@R-dplR] (a subtle work of staggering genius), plus `zoo` [@R-zoo] and `hydroTSM` [@R-hydroTSM] for the aggregation and gap-filling work later in the chapter. We also need `tidyverse` [@R-tidyverse] for wrangling, `tsibble` [@Wang2020tsibble] to hold the aggregated data, and `PNWColors` [@R-PNWColors] for the figure colors.
...
plot(zooRadApprox[1:100], type = "b", pch = 20, ylab = expression(W ~ m^-2), col = bookPal[1], lwd = 2)
lines(zooRadSpline[1:100], lwd = 2, col = bookPal[5], lty = "dotted")
