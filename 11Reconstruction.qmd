...
We hand-build the skill scores, fit the model with base R's `lm`, and use the `tidyverse` [@R-tidyverse] for wrangling and plotting. `slider` [@R-slider] gives us a clean centered running mean and `PNWColors` [@R-PNWColors] the palette. `forecast` [@R-forecast] gives us `ggAcf` to check the model's residuals, `nlme` [@R-nlme] gives us `gls` to confirm what that check shows, and `tseries` [@R-tseries] gives us `adf.test`/`kpss.test`, the same stationarity check from chapter four, to confirm the reconstruction itself.
