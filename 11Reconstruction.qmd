...
pProxyACF <- ggAcf(instrumental$Proxy) +
  labs(title = "Proxy data") +
  theme_minimal()

pFlowACF <- ggAcf(instrumental$ObsMAF) +
  labs(title = "Flow data") +
  theme_minimal()

cowplot::plot_grid(pProxyACF,
  pFlowACF,
  ncol = 2, align = "v")
```
...
glsModel <- gls(ObsMAF ~ Proxy, data = calib, correlation = corAR1())
summary(glsModel)
```
...
coRiverFlow <- coRiverFlow |>
  mutate(recon = predict(finalModel, coRiverFlow),
    reconSmooth = slide_dbl(recon, mean, .before = 12, .after = 12,
      .complete = TRUE))
```

The fitted relationship is ObsMAF $= `r round(coef(finalModel)[1], 1)` + `r round(coef(finalModel)[2], 1)` \times \text{proxy}$, in millions of acre-feet. Running it across the whole proxy gives an estimate of Colorado River flow for every year since 762. The annual reconstruction is noisy, so we overlay a 25-year centered running mean to bring out the slow swings, the droughts and pluvials that play out over decades.
...
ggplot(coRiverFlow, aes(Year)) +
  ...
  geom_line(aes(y = reconSmooth), color = sfPal[5], linewidth = 0.8) +
...
c(fullRecord = pctOfNormal(coRiverFlow$Year),
  ...
  dustBowl1933_1940 = pctOfNormal(1933:1940))
```

The Colorado River Compact, which still governs how the river is divided among seven states and Mexico, was negotiated in 1922 using flow data from the preceding decade. That decade, 1912 to 1921, ran at `r pctOfNormal(1912:1921)`% of the long-term average. The river was carved up on the basis of one of the wettest stretches in twelve hundred years, and the allocations have been writing checks the river cannot cash ever since. Meanwhile the full twelve-century record averages `r pctOfNormal(coRiverFlow$Year)`% of the instrumental mean, confirming that the gauged era itself ran a little wet. And the medieval megadrought of the twelfth century, 1130 to 1154, held the river at `r pctOfNormal(1130:1154)`% of normal for a quarter century straight, a sustained drought deeper and longer than anything in the instrumental record, deeper than the Dust Bowl. None of that is visible from ninety-nine years of gauge data. You can only see it because the trees remembered, and you can only believe the trees because the reconstruction earned it in verification.
