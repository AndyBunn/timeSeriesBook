...
Autocorrelation asked how a series relates to its own past. Cross-correlation asks the same question about two different series: how does $x$ at one time relate to $y$ at another? It is the bivariate version of the function we built in the autocorrelation chapter, and it earns its keep by finding the lag at which one series tracks another. That lag is the payoff. If river discharge peaks a few days after a storm, or grazers bloom a few weeks after the algae they eat, the cross-correlation will show it, and the offset is a clue to which series drives which. This is also the tool the last chapter pointed at: once you know that $x$ leads $y$, you have something to forecast $y$ with that is better than $y$'s own history.

The same warning from before applies. Two series that happen to share a trend, or a season, or even just strong memory of their own, will produce a fat cross-correlation that means nothing. The function will hand you a number whether or not there is anything between the two series, so a good cross-correlation analysis involves stripping out the structure each series carries on its own and then asking what is left. We will end the chapter on exactly that move.

## Packages

We stay in the tsibble idiom and read example data from CSV. `forecast` gives us `Ccf`, we lean on base R's `ar` and `filter` for the prewhitening at the end, and `PNWColors` supplies the book's palette for the figures.
...
lag_true <- 3
x_shifted <- dplyr::lag(x, lag_true)        # x from three steps back
y <- 0.8 * x_shifted + rnorm(n, sd = 0.5)
```

Always look at what you built. Here are the first sixty points of both series, and if you stare at the peaks you can almost see $y$ echoing $x$ a few steps later.
...
Line the two series up at the same time and there is almost nothing there. Line $y$ up against $x$ from three steps back and the relationship snaps into focus.
...
lagged <- tibble(x_lag3 = dplyr::lag(x, 3), y) |> drop_na()
cor(lagged$x_lag3, lagged$y)                # y against x from 3 steps back
```
...
#| label: ccf-lwa-raw
#| fig-height: 3
ggCcf(lwa$producers, lwa$grazers, lag.max = 24) +
  labs(title = "Producers and grazers, raw cross-correlation") +
  theme_minimal()
```

The strongest correlation is right at lag zero, the two series moving together within the month, which makes sense for organisms that respond to each other on a scale of weeks when you are only sampling monthly. But the plot is not symmetric. The bars just left of zero, where the producers lead, stand taller than the matching bars on the right, where they would trail. The algae running a month ahead of the grazers correlates better than the algae running a month behind, and that asymmetry is the food chain showing through. It is a softer signal than the planted spike, and that is the difference between a simulation and a lake. Plankton respond to each other faster than a monthly sample can resolve, so the lead gets squashed toward lag zero instead of standing off at a clean offset.
...
Here is the trap in its purest form. Take two random walks, each one nothing but a cumulative sum of independent noise, built so that they have absolutely nothing to do with each other.

```{r}
#| label: ccf-spurious-build
set.seed(70)
rw1 <- cumsum(rnorm(200))
rw2 <- cumsum(rnorm(200))
cor(rw1, rw2)
```

Why is a cumulative sum a random walk? Because that is the definition of one. Start at zero and at each step add a fresh independent shock to wherever you already are, $y_t = y_{t-1} + \epsilon_t$. Unwind that recursion and $y_t$ is nothing but the running total of every shock so far, $\epsilon_1 + \epsilon_2 + \cdots + \epsilon_t$, which is exactly what `cumsum(rnorm(n))` computes: draw the shocks, then accumulate them. Each value carries the whole history of the series inside it, so there is no mean to pull it back and it wanders wherever the accumulated shocks have taken it. We met this in the stationarity chapter as the unit root. Here it is the engine of the trap, and looking at the two walks shows you why.

```{r}
#| label: ccf-spurious-data
#| fig-height: 3
tibble(time = 1:200, rw1, rw2) |>
  pivot_longer(c(rw1, rw2)) |>
  ggplot(aes(time, value, color = name)) +
  geom_line() +
  scale_color_manual(values = c(rw1 = sfPal[1], rw2 = sfPal[5])) +
  labs(x = "Time", y = NULL, color = NULL) +
  theme_minimal()
```

Each one drifts in long smooth excursions, and over this particular stretch they happen to drift the same way. The correlation puts a number on that coincidence: `r round(cor(rw1, rw2), 2)`, between two series that share no mechanism whatsoever. The cross-correlation is worse, because it gets to go looking across lags for the best apparent match.

```{r}
#| label: ccf-spurious-raw
#| fig-height: 3
ggCcf(rw1, rw2, lag.max = 20) +
  labs(title = "Two independent random walks") +
  theme_minimal()
```

Bars towering over the dashed band at nearly every lag, all of it noise. Neither series knows the other exists. What fools the cross-correlation is that each series wanders slowly and smoothly, so any two long smooth wanders will drift in and out of step and look related over a stretch. This is the saboteur face of dependence again: the autocorrelation inside each series, the thing that makes it smooth, is exactly what manufactures the fake relationship between them.

The fix has a name, prewhitening, and the idea behind it is the spine of this whole book. Before you ask how two series relate, take the predictable structure out of each one and reduce it to white noise, so that what remains is only the part the series could not have produced on its own. For a random walk, the structure is the unit root, and the way to remove it is the way we removed it back in the stationarity chapter: difference it. Difference both series and the wandering is gone, leaving the independent shocks that drove each walk.
...
The forest of correlation collapses. Every bar is back inside the band, which is the correct answer: there was never anything between these two series, and once you stop letting their private wandering do the talking, the cross-correlation says so.
