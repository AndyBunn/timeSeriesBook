...
We stay in the `tsibble` [@Wang2020tsibble] framework, with `tidyverse` [@R-tidyverse] for wrangling, and read example data from CSV. `forecast` [@R-forecast] gives us `Ccf`, we lean on base R's `ar` and `filter` for the prewhitening at the end, and `PNWColors` [@R-PNWColors] supplies the book's palette for the figures.
...
planktonData |>
  ...
  geom_line() +
  scale_color_manual(values = c(producers = bookPal[1], grazers = bookPal[5])) +
...
tibble(time = 1:200, randomWalk1, randomWalk2) |>
  ...
  ggplot(aes(time, value, color = name)) +
  geom_line() +
  scale_color_manual(values = c(randomWalk1 = bookPal[1], randomWalk2 = bookPal[5])) +
  labs(x = "Time", y = NULL, color = NULL) +
  theme_minimal()
```
