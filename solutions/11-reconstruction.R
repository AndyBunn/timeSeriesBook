...
skill <- function(obs, pred, calibMean) {
  sse <- sum((obs - pred)^2)
  c(RE = 1 - sse / sum((obs - calibMean)^2),
...
for (wall in c(1930, 1955, 1980)) {
  ...
  cat(sprintf("wall %d | early->late: ", wall))
  print(round(validate(early, late), 3))
  cat(sprintf("wall %d | late->early: ", wall))
  print(round(validate(late, early), 3))
}
...
cat("\nmiddle -> both ends: ")
print(round(validate(mid, ends), 3))

# ---------------------------------------------------------------------------
# Exercise 2: Find the pluvials
# ---------------------------------------------------------------------------
finalModel <- lm(ObsMAF ~ Proxy, data = instrumental)
flow <- flow |>
  mutate(recon = predict(finalModel, flow),
