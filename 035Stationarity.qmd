...
The series stops coming home. Each shock gets passed forward at full strength and never fades, the variance grows without bound, and the thing wanders off and never settles. That failure has a name, a unit root, and it breaks the one property most tools in the rest of this book assume: stationarity. This chapter is about what it means, the canonical way it fails, the difference between a trend you can subtract and a trend you have to difference away, and how to test whether your data have earned the assumption you are about to make of them.

## Packages

We stay in the tsibble idiom from the earlier chapters. `forecast` gives us the ggplot-ready ACF we have been using, and `tseries` carries the two unit-root tests at the end of the chapter.
...
- The mean is constant. No trend, no drift, no slow climb. The series hovers around one level forever.
...
That is all we need. There is a stricter version that talks about the full joint distribution being invariant, and there is a measure-theoretic machinery underneath it, but the three constancies above are the working definition for everything we will do. When people say a series is stationary, this is usually what they mean.
...
e <- rnorm(n)
stat <- numeric(n)
for (i in 2:n) stat[i] <- 0.6 * stat[i - 1] + e[i]
...
tibble(t = 1:n, empirical = apply(walks, 1, var)) |>
  ggplot(aes(t)) +
  geom_line(aes(y = empirical), color = "steelblue") +
...
for (i in 2:n) stoch_trend[i] <- 0.05 + stoch_trend[i - 1] + rnorm(1)

tibble(t, `Deterministic trend` = det_trend,
...
#| label: stat-detrend
#| fig-height: 3
det_resid <- resid(lm(det_trend ~ t))
ggplot(tibble(t, det_resid), aes(t, det_resid)) +
  geom_line() +
...
#| fig-height: 3
stoch_resid <- resid(lm(stoch_trend ~ t))
ggplot(tibble(t, stoch_resid), aes(t, stoch_resid)) +
  geom_line() +
...
Subtracting the line did not fix it. The residuals still wander in long slow excursions, because there was no line there to remove in the first place. The lag-1 autocorrelation of those residuals is still about `r round(acf(stoch_resid, plot = FALSE)$acf[2], 2)`, almost as persistent as the original. Detrending a stochastic trend leaves you with a non-stationary series and a false sense that you handled it.
...
#| fig-height: 3
stoch_diff <- diff(stoch_trend)
ggplot(tibble(t = 2:n, stoch_diff), aes(t, stoch_diff)) +
  geom_line() +
...
#| fig-height: 3
planted_diff <- diff(planted)
ggplot(tibble(t = seq_along(planted_diff), planted_diff), aes(t, planted_diff)) +
  geom_line() +
  labs(x = "Time", y = expression(y[t] - y[t-1]),
...
#| warning: false
adf.test(planted_diff)
kpss.test(planted_diff)
```
...
n <- 250
t <- 1:n
mystery_a <- 0.04 * t + arima.sim(list(ar = 0.5), n = n)
mystery_b <- cumsum(rnorm(n)) + 0.04 * t
```
