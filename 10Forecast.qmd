...
We stay with `tsibble` [@Wang2020tsibble], `tidyverse` [@R-tidyverse] for wrangling, and `PNWColors` [@R-PNWColors] for the figure colors. `forecast` [@R-forecast] gives us the AR fitting and prediction tools and a tidy autoplot, and we read example data from CSV as usual.
...
ggplot() +
  ...
  geom_ribbon(data = forecastDf,
    ...
    fill = bookPal[1], alpha = 0.3) +
  geom_line(data = forecastDf, aes(time, yhat), color = bookPal[1]) +
...
ggplot() +
  ...
  geom_ribbon(data = backwardDf,
    aes(time, ymin = yhat - sd, ymax = yhat + sd),
    fill = bookPal[5], alpha = 0.3) +
  geom_line(data = backwardDf, aes(time, yhat), color = bookPal[5]) +
  labs(x = "Time", y = expression(y[t]),
...
ggplot() +
  ...
  geom_ribbon(data = forecastTb, aes(year, ymin = lower, ymax = upper),
    fill = bookPal[1], alpha = 0.4) +
  geom_line(data = forecastTb, aes(year, yhat), color = bookPal[1]) +
  labs(x = "Year", y = "Sunspots (n)") +
...
ggplot() +
  ...
  geom_line(data = arTb, aes(year, yhat), color = bookPal[1]) +
  labs(x = "Year", y = "Recruits (thousands)",
...
ggplot() +
  geom_line(data = testTb, aes(year, rec)) +
  geom_line(data = hwTb, aes(year, yhat), color = bookPal[5]) +
  labs(x = "Year", y = "Recruits (thousands)",
