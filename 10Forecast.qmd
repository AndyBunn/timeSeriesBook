...
rec_train <- head(rec, length(rec) - h)
rec_test  <- tail(rec, h)
```
...
rec_ar <- ar(rec_train)
rec_ar$order
ar_fc <- predict(rec_ar, n.ahead = h)
```
...
#| echo: false
#| fig-height: 3.5
test_tb <- tibble(year = time(rec_test) |> as.numeric(),
                  rec = as.numeric(rec_test))
ar_tb <- tibble(year = test_tb$year, yhat = as.numeric(ar_fc$pred))
ggplot() +
  geom_line(data = test_tb, aes(year, rec)) +
  geom_line(data = ar_tb, aes(year, yhat), color = "darkgreen") +
  labs(x = "Year", y = "Recruits (thousands)",
       title = "AR forecast (green) against the withheld decade (black)") +
  theme_minimal()
```
...
ar_rmse <- sqrt(mean((ar_fc$pred - rec_test)^2))
ar_cor  <- cor(ar_fc$pred, rec_test)
c(order = rec_ar$order, rmse = ar_rmse, cor = ar_cor)
```

Now ask what an idiot could score. There is no season to hand a baseline for free here, so the fair baseline is simpler still: predict every withheld year with the training period's long-run average recruitment, full stop.

```{r}
#| label: fc-halibut-mean
mean_pred <- rep(mean(rec_train), h)
mean_rmse <- sqrt(mean((mean_pred - rec_test)^2))
c(ar_rmse = ar_rmse, mean_rmse = mean_rmse)
```

The AR(1) beats the flat average by a wide margin this time, not a rounding error. That is the opposite lesson from a seasonal river: there, almost all the apparent skill was a free season and the AR barely cleared the baseline. Here there is no free season to inflate the comparison, so the improvement the model shows is actual signal, persistence in how one year's recruitment leans on the last. It is a smaller, humbler forecast than the sunspot cycle, and it is exactly the kind of number a stock assessment leans on, an unglamorous edge over the flat average that still changes what catch limit looks defensible.
...
```{r}
#| label: fc-hw-fit
rec_hw <- HoltWinters(rec_train, gamma = FALSE)
hw_fc <- predict(rec_hw, n.ahead = h)
hw_rmse <- sqrt(mean((hw_fc - rec_test)^2))
c(ar_rmse = ar_rmse, hw_rmse = hw_rmse, mean_rmse = mean_rmse)
```
