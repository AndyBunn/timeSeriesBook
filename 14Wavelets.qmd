...
`dplR` [@R-dplR] supplies `morlet()` and `wavelet.plot()` for the continuous wavelet transform, built on the algorithm from @TorrenceCompo1998. `waveslim` [@R-waveslim] supplies `mra()` for the discrete version. `tidyverse` [@R-tidyverse] and `PNWColors` [@R-PNWColors] handle the planted demo.
