...
We stay in the `tsibble` [@Wang2020tsibble] framework from the earlier chapters, with `tidyverse` [@R-tidyverse] and `PNWColors` [@R-PNWColors] as usual. `forecast` [@R-forecast] gives us the ggplot-ready ACF we have been using, and `tseries` [@R-tseries] carries the two unit-root tests at the end of the chapter.
