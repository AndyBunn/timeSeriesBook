...
This aside explores why the autocorrelation function (ACF) and partial autocorrelation function (PACF) behave the way they do in MA(1) processes. We focus on two unintuitive and commonly misunderstood features:
...
So once the regression adjusts for $y_{t-1}$, the residual effect of $y_{t-2}$ is actually in the **opposite direction**, resulting in a negative coefficient. The strength of this negative partial correlation increases with $\theta$, leading to a nearly linear negative slope, as our plot confirms.
...
The notation $\phi_{22}$ refers to the partial autocorrelation at lag 2, the coefficient on $y_{t-2}$ in a regression of $y_t$ on both $y_{t-1}$ and $y_{t-2}$. More generally, $\phi_{kk}$ denotes the PACF at lag $k$, which is what the Durbin-Levinson recursion (see the [aside on PACF by hand](04xAsidePACFByHand.html)) is actually computing when it estimates partial autocorrelations: the unique contribution of $y_{t-k}$ to predicting $y_t$ after controlling for all the intermediate lags $y_{t-1}, y_{t-2}, \ldots, y_{t-(k-1)}$. This negative value at lag 2 reflects how the MA(1) structure produces a compensating reversal in direction once the influence of $y_{t-1}$ is accounted for.

This matches the shape in our simulation, a negative curve that grows more negative with larger $\theta$.

## Where This Shows Up

This simulation-driven analysis shows two things: PACF lag 1 underestimates $\theta$ because it captures indirect correlation from shared noise, not a structural relationship, and PACF lag 2 is consistently negative because of how overlapping shocks in an MA(1) structure reverse direction once you've accounted for the intermediate lag. Both patterns are the reason the ARMA chapter can tell you, without deriving it there, that an MA process's PACF decays and oscillates instead of cutting off cleanly the way an AR process's does. Recognizing that shape helps you read an ACF/PACF pair correctly and avoids the common misstep of treating a PACF value as a direct stand-in for a model parameter.
