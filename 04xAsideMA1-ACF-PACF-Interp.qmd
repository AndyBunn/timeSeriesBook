...
Let's build one MA(1) series and look at it.
...
names(res) <- c("simulatedTheta", "pacfLag1", "pacfLag2")
```
...
```{r}
#| label: ma1-lag1-plot
ggplot(res, aes(x = simulatedTheta, y = pacfLag1)) +
...
```{r}
#| label: ma1-lag2-vs-lag1-plot
ggplot(res, aes(x = pacfLag1, y = pacfLag2)) +
  geom_point(alpha = 0.4, color = "gray") +
  labs(
    x = "PACF Lag 1",
    y = "PACF Lag 2",
...
```{r}
#| label: ma1-lag2-vs-theta-plot
# Also show vs theta
ggplot(res, aes(x = simulatedTheta, y = pacfLag2)) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point(alpha = 0.4, color = "gray") +
  geom_smooth(method = "lm") +
  labs(
    x = expression(True ~ theta[1]),
    y = expression(PACF ~ Lag ~ 2 ~ (hat(phi)[22])),
