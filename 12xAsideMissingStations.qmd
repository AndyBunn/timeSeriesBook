...
donor <- read_csv("data/bellingham_3ssw_temp.csv") |> rename(donor_tavg = TAVG)

full_join(kbli, donor, by = "DATE") |>
  filter(DATE >= as.Date("1993-01-01"), DATE <= as.Date("2001-12-01")) |>
  pivot_longer(c(TAVG, donor_tavg), names_to = "series", values_to = "tavg") |>
  mutate(series = recode(series, TAVG = "Bellingham Airport (kbli)",
                          donor_tavg = "Bellingham 3 SSW (donor)")) |>
  ggplot(aes(DATE, tavg, color = series)) +
  geom_line() +
  scale_color_manual(values = c("Bellingham Airport (kbli)" = sfPal[1],
...
ggplot(overlap, aes(donor_tavg, TAVG)) +
  geom_point(color = "grey40", alpha = 0.6) +
  geom_smooth(method = "lm", se = FALSE, color = sfPal[5]) +
  labs(x = expression("Bellingham 3 SSW monthly " * degree * C),
...
```{r}
#| label: gap-fit-check
overlapFit <- lm(TAVG ~ donor_tavg, data = overlap)
...
validateFit <- lm(TAVG ~ donor_tavg, data = trainSet)
testPred <- predict(validateFit, newdata = testSet)
rmseRegression <- sqrt(mean((testPred - testSet$TAVG)^2))

climLookup <- trainSet |> mutate(mo = month(DATE)) |> group_by(mo) |> summarise(climMean = mean(TAVG))
...
tibble(method = c("donor regression", "climatology baseline"),
       rmse_degC = c(rmseRegression, rmseClimatology))
```
...
```{r}
#| label: gap-fill
finalFit <- lm(TAVG ~ donor_tavg, data = overlap)

gapFilled <- kbli |>
  filter(is.na(TAVG)) |>
  select(DATE) |>
  inner_join(donor, by = "DATE") |>
  mutate(TAVG = predict(finalFit, newdata = tibble(donor_tavg = donor_tavg)),
         source = "filled")
