...
A moving-average model is built entirely out of shocks, so it is worth recalling the word from the [autocorrelation chapter](03TimeSeries-Autocorrelation.html). A shock is the disturbance $\epsilon_t$, the unpredictable input that hits the system at time $t$. If you would rather not borrow the econometrics word, read it as a disturbance or a forcing, the flood or the fire the past did not see coming.
:::
...
We now have a model for the dependence in a stationary series. The obvious next question is what it lets us do, and the headline answer is forecasting: if today leans on yesterday, then today tells us something about tomorrow. It also tells us how fast that something runs out. In the next chapter we push an AR(1) forward and watch the uncertainty in the forecast grow shock by shock, as a geometric series that saturates at a ceiling set by $\phi$. The same coefficient that governs the ACF decay governs how far ahead the model can usefully see.
