...
We stay in the `tsibble` [@Wang2020tsibble] framework from the earlier chapters, with `tidyverse` [@R-tidyverse] and `PNWColors` [@R-PNWColors] as usual. `forecast` [@R-forecast] gives us the ggplot-ready ACF and PACF and the `Arima` fitting function, `gridExtra` [@R-gridExtra] lays a few plots out together, and `tseries` [@R-tseries] carries the ADF and KPSS tests we met last chapter.
