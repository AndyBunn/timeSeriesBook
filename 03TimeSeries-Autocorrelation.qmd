...
To see the whole autocorrelation structure at once, you compute $\rho$ at successive lags and plot it. That plot is called a correlogram, and the function `Acf` draws it. We use `Acf` from `forecast` rather than the base `acf`; they are nearly identical (see `?Acf` for the small differences).
...
You will often want the numbers, not just the picture. They live inside the object `Acf` returns; `str` is, as always, your friend for finding them.
...
Note the 1.0 at lag zero. Every series is perfectly correlated with itself at no lag, so that bar is always there and always one. I find it a little annoying, but that is the convention.

Because we are living in the tidyverse, it is worth knowing that `forecast` also ships a ggplot version, `ggAcf`, which we will lean on from here.
...
For an AR(1) process, once you account for the lag-1 relationship there is nothing left to explain at higher lags, so the partial autocorrelation drops to near zero after lag 1. That gives the pair of signatures you will use constantly: an AR(1) process has an ACF that decays geometrically and a PACF that cuts off sharply after lag 1. If instead the PACF is the one that decays slowly, you may be looking at a moving-average process, which we meet next chapter. For why an MA process flips the picture, see the [aside on MA(1) ACF and PACF shapes](03xAsideMA1-ACF-PACF-Interp.html).

If you want to see that the partial autocorrelation really is just a stack of regression coefficients, with the `Pacf` function reproduced from scratch three different ways, that is the [aside on the PACF by hand](03xAsidePACFByHand.html). It is optional, but it demystifies the function.
...
Simulated series are clean by construction. Let's look at a few series that ship with R and see how the tools behave on data that nobody designed.
